...
def expon_lr_helper(step, lr_init, lr_final, lr_delay_steps, lr_delay_mult, max_steps):
    """
    최상위 레벨에서 정의된 함수이므로, pickle 가능.
    step과 여러 파라미터를 받아 get_expon_lr_func의 helper와 동일한 로직을 수행함.
    """
    if step < 0 or (lr_init == 0.0 and lr_final == 0.0):
        return 0.0
...
def get_expon_lr_func(
    ...
    """
    - 기존의 helper 함수를 expon_lr_helper(전역 함수)로 분리
    - partial을 사용하여 lr_init, lr_final 등 고정 파라미터를 미리 세팅
    - 반환되는 객체는 '함수처럼' step을 인자로 받을 수 있고, pickle도 가능
    """
    # partial을 사용해, step을 제외한 나머지 인수를 미리 세팅해둠
    return partial(
        expon_lr_helper,
        lr_init=lr_init,
        lr_final=lr_final,
