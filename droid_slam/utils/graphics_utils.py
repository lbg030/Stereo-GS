...
def depth_fusion(stereo_depth, slam_depth, fussion_coeff, mask = None):
    fused_depth = torch.zeros_like(stereo_depth)
    fused_depth[mask] = stereo_depth[mask]
    fused_depth[~mask] = (1-fussion_coeff) * stereo_depth[~mask] + fussion_coeff * slam_depth[~mask]
