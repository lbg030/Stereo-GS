...
def JDSA(target, weight, eta, poses, disps, intrinsics, disps_prior, ii, jj, dscales=None, alpha=None):
    ...
    # mscales_bi, Jbi = droid_backends.bi_inter(scales, grid)
    # disps_bi = depth_prior * mscales_bi
    disps_bi, Jbi = get_prior_depth_aligned(disps_prior, dscales[kx])

    # rd is defference between droid disps and aligned disps
    rd = (disps[0,kx] - disps_bi).view(-1, ht*wd)
    Jd = torch.ones_like(rd).view(1, -1, 1, ht*wd)
    # Jd = (-1. / (disps[0,kx] ** 2)).view(1, -1, 1, ht*wd)

    # m : disps_prior > 0 ( valid prior )
    # disps_prior : 1 / extracted_mono_depth
    # Jbi : weights for each pixel
