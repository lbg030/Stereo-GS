...
class SLAM_GUI:
    ...
    def init_widget(self):
        ...
        self.window = gui.Application.instance.create_window(
            "Stereo_GS", self.window_w, self.window_h
        )
    ...
    def add_camera(self, camera, name, color=[0, 1, 0], gt=False, size=0.01):
        W2C = (
            getWorld2View3(camera.R_gt, camera.T_gt)
            if gt
            else getWorld2View3(camera.R, camera.T)
        )
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.gtdepth is not None:
            ...
            depth = gaussian_packet.gtdepth.squeeze(0).detach().cpu().numpy()
            depth = imgviz.depth2rgb(
                depth, min_value=0.1, max_value=50.0, colormap="jet"
            )
    ...
    def render_gui(self):
        ...
        if torch.sum(torch.isnan(current_cam.R)) == 0:
