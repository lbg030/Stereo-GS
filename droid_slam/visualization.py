...
def droid_visualization(video, device="cuda:0"):
    ...
    droid_visualization.filter_thresh = 0.1
    ...
    def animation_callback(vis):
        ...
        with torch.no_grad():
            ...
            images = images.cpu()[:,0, [2,1,0],3::8,3::8].permute(0,2,3,1) / 255.0
            # images = images.cpu()[:, 0, [2,1,0], ...].permute(0,2,3,1) / 255.0
            ...
            # count = droid_backends.depth_filter(
            #     video.poses, video.disps_up, video.intrinsics[0], dirty_index, thresh)
            count = count.cpu()

            disps = disps.cpu()
            masks = ((count >= 1) & (disps > .5*disps.mean(dim=[1,2], keepdim=True)))
            # video.droid_masks[dirty_index] = masks.cuda()
