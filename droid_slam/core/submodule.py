...
class FlashMultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, window_size=(-1,-1)):
        ...
        Q = Q.view(B, L, self.num_heads, self.head_dim).contiguous()
        K = K.view(B, L, self.num_heads, self.head_dim).contiguous()
        V = V.view(B, L, self.num_heads, self.head_dim).contiguous()

        Dh = Q.shape[-1]

        use_mea = (
            xops is not None and
            attn_mask is None and
            window_size == (-1, -1) and
            Dh % 8 == 0 and Dh >= 8  # ← 핵심 조건
        )
        ...
        if use_mea:
            ...
            # SDPA 폴백
            Bh = B * self.num_heads
            q = Q.permute(1, 0, 2, 3).reshape(L, Bh, Dh)
            k = K.permute(1, 0, 2, 3).reshape(L, Bh, Dh)
            v = V.permute(1, 0, 2, 3).reshape(L, Bh, Dh)
            out = F.scaled_dot_product_attention(q, k, v, dropout_p=0.0, is_causal=False)
            attn_output = out.reshape(L, B, self.num_heads, Dh).permute(1, 0, 2, 3)
