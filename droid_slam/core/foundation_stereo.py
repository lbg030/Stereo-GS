...
from droid_slam.foundation_Utils import *
import time
...
class FoundationStereo(nn.Module):
    ...
    def upsample_disp(self, disp, mask_feat_4, stem_2x):
        ...
        with torch.amp.autocast('cuda', enabled=self.args.mixed_precision):  # For some reason, with amp, the model goes OOM
    ...
    def forward(self, image1, image2, iters=12, flow_init=None, test_mode=False, low_memory=False, init_disp=None):
        ...
        non_convergence = torch.zeros_like(disp).squeeze().cuda()
        for itr in range(iters):
            ...
            non_convergence = (itr+1)/iters * delta_disp.squeeze()

            # upsample predictions
            disp_up = self.upsample_disp(disp.float(), mask_feat_4.float(), stem_2x.float())
            disp_preds.append(disp_up)

            non_convergence = self.upsample_disp(non_convergence.unsqueeze(0).unsqueeze(0).float(), mask_feat_4.float(), stem_2x.float())
            threshold = torch.quantile(non_convergence.float(), 0.95)
            mask = non_convergence < threshold

        if test_mode:
            return disp_up, mask.squeeze().detach().cpu().numpy()
